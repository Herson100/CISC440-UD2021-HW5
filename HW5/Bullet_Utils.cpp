...
void copy_flocker_states_to_graphics_objects()
{
  ...
  for (int i = 0; i < num_flockers; i++) {
    ...
    xyz_velocities[i] = glm::vec3(flocker_array[i]->new_velocity.x * 75, flocker_array[i]->new_velocity.y * 75, flocker_array[i]->new_velocity.z * 75);
...
void bullet_add_obstacles()
{
  ...
  btCollisionShape* rightShape = new btStaticPlaneShape(btVector3(-1, 0, 0), -9);
