...
extern vector<double> predator_flocker_squared_distance;
extern Predator *pred;
...
Flocker::Flocker(int _index,
    ...
    double min_f_distance, double max_f_distance, double f_weight,
...
{ 
  ...
  fear_weight = f_weight;

  min_fear_distance = min_f_distance;
  min_squared_fear_distance = min_fear_distance * min_fear_distance;

  max_fear_distance = max_f_distance;
...
bool Flocker::compute_fear_force() {
    ...
    for (int i = 0; i < predator_flocker_squared_distance.size(); i++) {
        if (predator_flocker_squared_distance[i] >= min_squared_fear_distance &&
            predator_flocker_squared_distance[i] <= max_squared_fear_distance) {
            // set (unweighted) force magnitude

            F = glm::normalize(max_squared_fear_distance - predator_flocker_squared_distance[i]) *
                glm::normalize(max_squared_fear_distance - predator_flocker_squared_distance[i]);

            // set force direction

            direction = -((float)F * glm::normalize(flocker_array[i]->position - pred->position));
