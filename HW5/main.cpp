...
void initialize_flocking_simulation()
{
  ...
  predator_flocker_squared_distance.resize(num_flockers);
  flocker_array.clear();
  ...
  for (int i = 0; i < num_flockers; i++) {
      flocker_array.push_back(new Flocker(i,
          ...
          0.01, 0.1, 0.001,                        // min, max fear distance, weight
    ...
    pred = new Predator(i,
        ...
        0.002,            // randomness
        1.0, 1.0, 1.0,
        1.0, 1.5, uniform_random(0.0005, 0.002), // min, max hunger distance, weight
        flocker_history_length);
  }
}
...
void update_flocking_simulation()
{
  ...
  // and predator-flocker distances

  calculate_flocker_squared_distances();
  calculate_predator_flocker_squared_distances();
  ...
  // get new_position, new_velocity for each predator
  ...
  pred->finalize_update(box_width, box_height, box_depth);
}
