...
Predator * pred;
vector <double> predator_flocker_squared_distance; // distances between predator and flockers
...
void calculate_predator_flocker_squared_distances()
{
    int i;
    glm::vec3 diff;

    for (i = 0; i < flocker_array.size(); i++){
        diff = pred->position - flocker_array[i]->position;
        predator_flocker_squared_distance[i] = glm::length2(diff);
    }
}
...
Predator::Predator(int _index,
		 ...
		 int max_hist) : Creature(_index, init_x, init_y, init_z, init_vx, init_vy, init_vz, r *.5 , g * .5, b * .5, max_hist)
{ 
...
void Predator::draw(glm::mat4 Model)
{
    ...
    if (predator_draw_mode == DRAW_MODE_HISTORY) {
        ...
        color_buffer_data[0] = 0.0f;
        color_buffer_data[1] = 1.0f;
        color_buffer_data[2] = 1.0f;

        color_buffer_data[3] = 0.0f;
        ...
        color_buffer_data[13] = 1.0f;
        color_buffer_data[14] = 0.0f;

        color_buffer_data[15] = 1.0f;
        color_buffer_data[16] = 1.0f;
...
bool Predator::compute_hunger_force()
{
    ...
    for (int i = 0; i < predator_flocker_squared_distance.size(); i++) {
        if (predator_flocker_squared_distance[i] >= min_squared_hunger_distance &&
            predator_flocker_squared_distance[i] <= max_squared_hunger_distance) {
            // set (unweighted) force magnitude

            F = glm::normalize(max_squared_hunger_distance - predator_flocker_squared_distance[i]) *
                glm::normalize(max_squared_hunger_distance - predator_flocker_squared_distance[i]);

            // set force direction

            direction = (float)F * glm::normalize(flocker_array[i]->position - pred->position);   // opposite direction of separation
...
void Predator::update()
{
    ...
    //printf("%f, %f, %f", hunger_force[0], hunger_force[1], hunger_force[2]);
